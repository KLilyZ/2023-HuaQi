...
#设定初始值
date='2010-01-01~2015-12-31'#这里可以根据自己所要的时间跨度自行修改
start=11#自己设定从firm_message.csv的第几行开始
end=15#自己设定从firm_message.csv的第几行结束,建议每200/300家公司爬取一次
...
with open('ndbg_data.csv', mode='a', encoding='utf-8',newline='') as f:
    ...
    for err,i in enumerate(range(start-2, end-1)):
        ...
        try:
            ...
            for page in range(1,total_pages+1):
                print(f'正在下载{i+1}-{code}-{zwjc} 第{page}/{total_pages}页年报信息')
                post_data={
                    'pageNum': f'{page}',
                    'pageSize': '30',
                    'column': 'szse',
            ...
            ws.cell(err+1,1).value=f'{code}有误，请进一步确认'
wb.save('错误记录.xlsx')
