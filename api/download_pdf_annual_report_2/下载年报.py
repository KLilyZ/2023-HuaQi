...
def download_annual_report():
    ...
    root = pathroot + r'\DATA SUPPORT\词频统计\年报pdf版本'  # 存放pdf年报的位置
    root2 = pathroot + r'\DATA SUPPORT\词频统计\年报txt版本'  # 存放txt年报的位置
    start = 2  # merged_data.xlsx文件中的第几行开始
    end = 3000  # merged_data.xlsx文件中的第几行结束
    RemoveDir(root)
    RemoveDir(root2)

    wb = openpyxl.load_workbook(os.path.dirname(__file__)+"\\" + 'merged_data.xlsx')
    ws = wb.active
    ...
    wb.save(os.path.dirname(__file__) + 'merged_data.xlsx')
